...
def setup_registers(architecture):
    ...
    for index, register in enumerate(reversed(architecture["register_list"])):
        location = "randomized_state_data"
...
def save_a_stack_word(architecture, location, offset, stack_offset, register, scratch_register):
    local_dictionary = {
        "location": location,
        "offset": offset,
        ...
        "register": register,
        "scratch_register": scratch_register,
        "save_a_register": save_a_register(
            architecture,
            location,
            offset,
            register,
    ...
    jinja_template = jinja2.Environment().from_string(
        architecture["templates"]["save_a_stack_word"]
    )
    return jinja_template.render(local_dictionary)


def save_the_state(architecture, config, location_index):
    register_list = architecture["register_list"]
    assert len(register_list) > 1

    location = "printable_data_" + str(location_index)
    register = architecture["stack_pointer"]
    scratch_register = register_list[0]
    current = save_a_register(architecture, location, 0, register, scratch_register)
    result = current + "\n"

    register_size = architecture["register_size"]
    for index, register in enumerate(register_list):
        offset = (index + 1) * register_size
        scratch_register = register_list[0] if index != 0 else register_list[1]
        current = save_a_register(architecture, location, offset, register, scratch_register)
        result = result + current + "\n"

    offset_delta = offset + register_size
    register = register_list[0]
    scratch_register = register_list[1]
    for offset in range(register_size, config["stack_byte_count"] + register_size, register_size):
        current = save_a_stack_word(
            architecture, location, offset + offset_delta, offset, register, scratch_register
        )
        result = result + current + "\n"
...
def render_function_description(jinja_env, architectures, config, functions, out_dir):
    for architecture_name, architecture in architectures.items():
        dictionary = {
            ...
            "save_the_state": lambda n, a=architecture, c=config: asm(save_the_state(a, c, n)),
